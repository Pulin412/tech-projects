import {authenticate} from "./get-auth-token.js";
...
export const options = {
    ...
    scenarios: {
        ...
        like_user: {
            ...
            timeUnit: '1s',
            stages: [
                {target: 20, duration: '15s'}, // go from 10 to 20 iters/s in the first 30 seconds
                {target: 20, duration: '7s'}, // hold at 20 iters/s for 1 minute
            ...
            tags: {test_type: 'like_user'}, // different extra metric tags for this scenario
            exec: 'like_user', // same function as the scenario above, but with different env vars
        }
    }
};
 ...
 * uc1
 */
export function like_user(data) {
    ...
    if(response.status !== 200) {
        console.log(`operation: like_user, url: ${url}, Status:${response.status}`);
    }
